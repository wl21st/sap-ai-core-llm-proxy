# Spec: Streaming Default Behavior

## Purpose

Defines how the proxy handles the `stream` parameter when it is absent or explicitly set in request bodies. The proxy SHALL default to non-streaming behavior (stream=false) when the parameter is absent, in accordance with the Anthropic Messages API and OpenAI Chat Completions API specifications.
