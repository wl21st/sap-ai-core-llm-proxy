...
Migrating to the standard Python `src/` layout with `saip` as the top-level package, switching the build backend to `hatchling` (Astral's recommended backend for `uv`), reorganizing scripts and tests into standard directories, and modernizing the Makefile creates a robust, industry-standard packaging architecture.

## What Changes

- **Adopt `src/` Layout (`src/saip/`)**: Relocate internal packages (`auth`, `config`, `handlers`, `routers`, `utils`) and core modules (`main`, `cli`, `load_balancer`, `proxy_helpers`, `version`, `proxy_server`) under `src/saip/`.
...
- **Top-Level Package Namespace**: Establish `saip` as the clean import namespace (e.g. `from saip.config import ProxyConfig`, `from saip.auth import RequestValidator`).
...
### New Capabilities
- `src-package-layout`: Defines the standard `src/saip/` package structure and isolated namespace for all application modules.
...
**Code affected:**
- `src/saip/` - Created containing `auth/`, `config/`, `handlers/`, `routers/`, `utils/`, `main.py`, `cli.py`, `version.py`, `load_balancer.py`, `proxy_helpers.py`, `proxy_server.py`.
- `scripts/` - Created containing `inspect_deployments.py`, `load_testing.py`, `test_mmyydd_logging.py`, `test_yyyymmdd_logging.py`.
- `pyproject.toml` - Updated build-system (`hatchling`), script entrypoint (`sap-ai-proxy = "saip.main:main"`), and tool configs.
- `pytest.ini` - Removed (merged into `pyproject.toml`).
- `tests/` - Internal test imports updated to `saip.*`, loose test files moved to `tests/unit/`.
