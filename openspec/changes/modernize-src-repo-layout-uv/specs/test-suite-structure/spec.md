...
The test configuration SHALL measure code coverage across all package modules in `src/saip/`, including routers, handlers, auth, config, and utils.

#### Scenario: Generating test coverage reports
- **WHEN** running `pytest --cov=saip`
