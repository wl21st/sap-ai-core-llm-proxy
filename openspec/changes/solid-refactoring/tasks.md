...
- [x] 1.1 Create `converters/` directory structure with `__init__.py`
- [x] 1.2 Create `converters/mappings.py` with centralized stop reason mappings and API version constants
- [x] 1.3 Create `converters/base.py` with `Converter` Protocol definition
- [x] 1.4 Update existing code to import constants from `converters.mappings` (remove duplicates from `proxy_helpers.py`, `streaming_generators.py`, `streaming_handler.py`)

## 2. Extract Converters - OpenAI Module

- [x] 2.1 Create `converters/openai.py` with `from_claude()` function (extract from `Converters.convert_claude_to_openai`)
- [x] 2.2 Add `from_claude37()` function (extract from `Converters.convert_claude37_to_openai`)
- [x] 2.3 Add `from_gemini()` function (extract from `Converters.convert_gemini_to_openai`)
- [x] 2.4 Write unit tests for `converters/openai.py`

## 3. Extract Converters - Claude Module

- [x] 3.1 Create `converters/claude.py` with `from_openai()` function (extract from `Converters.convert_openai_to_claude`)
- [x] 3.2 Add `from_openai_messages()` function (extract from `Converters.convert_openai_messages_to_claude`)
- [x] 3.3 Add `to_bedrock()` function (extract from `Converters.convert_claude_to_bedrock`)
- [x] 3.4 Write unit tests for `converters/claude.py`

## 4. Extract Converters - Gemini Module

- [x] 4.1 Create `converters/gemini.py` with `from_openai()` function (extract from `Converters.convert_openai_to_gemini`)
- [x] 4.2 Add `from_claude()` function (extract from `Converters.convert_claude_to_gemini`)
- [x] 4.3 Write unit tests for `converters/gemini.py`

## 5. Extract Converters - Chunks Module

- [x] 5.1 Create `converters/chunks.py` with `claude_to_openai_chunk()` function
- [x] 5.2 Add `claude37_to_openai_chunk()` function
- [x] 5.3 Add `gemini_to_openai_chunk()` function
- [x] 5.4 Write unit tests for `converters/chunks.py`

## 6. Update Facade and Exports

- [x] 6.1 Update `converters/__init__.py` to re-export all converter functions
- [x] 6.2 Update `proxy_helpers.py:Converters` to delegate to new modules (backward compatibility)
- [x] 6.3 Verify all existing tests pass with delegation layer

## 7. Model Handler Registry

- [x] 7.1 Create `handlers/registry.py` with `ModelHandlerRegistry` class and `ModelHandler` Protocol
- [x] 7.2 Create `handlers/base_handler.py` with `DefaultHandler` implementation
