...
def handle_embedding_request():
    ...
    try:
        endpoint_url, modified_payload, subaccount_name = handle_embedding_service_call(input_text, model,
                                                                                        encoding_format)
...
def fetch_token(subaccount_name: str) -> str:
    ...
    with subaccount.token_info.lock:
        ...
        try:
            ...
            logging.error(
                f"HTTP error fetching token for '{subaccount_name}': {err.response.status_code}-{err.response.text}")
            subaccount.token_info.token = None
            subaccount.token_info.expiry = 0
            raise ConnectionError(
                f"HTTP Error {err.response.status_code} fetching token for '{subaccount_name}'") from err
...
def verify_request_token(request):
    ...
    logging.info(
        f"verify_request_token, Token received in request: {token[:15]}..." if token and len(token) > 15 else token)
    ...
    try:
        ...
        openai_response = {
            ...
            "usage": {
                ...
                "total_tokens": response.get("usage", {}).get("input_tokens", 0) + response.get("usage", {}).get(
                    "output_tokens", 0)
            }
        }
...
def convert_claude37_chunk_to_openai(claude_chunk, model_name):
    ...
    try:
        ...
        error_payload = {
            ...
            "model": model_name,
            "choices": [
                {"index": 0, "delta": {"content": f"[PROXY ERROR: Failed to convert upstream chunk - {str(e)}]"},
                 "finish_reason": "stop"}]
        }
...
def is_claude_model(model):
    return any(
        keyword in model for keyword in ["claude", "clau", "claud", "sonnet", "sonne", "sonn", "CLAUDE", "SONNET"])
...
def is_gemini_model(model):
    ...
    """
    return any(
        keyword in model.lower() for keyword in ["gemini", "gemini-1.5", "gemini-2.5", "gemini-pro", "gemini-flash"])
...
def proxy_claude_request_original():
    ...
    if not model:
        return jsonify(
            {"type": "error", "error": {"type": "invalid_request_error", "message": "Missing 'model' parameter"}}), 400
    ...
    try:
        ...
        if is_gemini_model(model):
            ...
            api_version = "2024-12-01-preview" if any(
                m in model for m in ["o3", "o4-mini", "o3-mini"]) else "2023-05-15"
        ...
        return jsonify(
            {"type": "error", "error": {"type": "api_error", "message": "An unexpected error occurred."}}), 500
                        ...
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if chunk_type == "metadata":
                                    ...
                                    logging.debug(
                                        f"Received Claude 3.7 metadata from '{subaccount_name}': {claude_metadata}")
                                    continue
                                ...
                                logging.error(f"Error processing Claude 3.7 chunk from '{subaccount_name}': {e}",
                                              exc_info=True)
                                error_payload = {
                                    "id": f"chatcmpl-error-{random.randint(10000, 99999)}",
                                    "object": "chat.completion.chunk",
                    ...
                    if line_bytes:
                        ...
                        if line_content and line_content != "[DONE]":
                            try:
                                ...
                                logging.error(f"Error processing Gemini chunk from '{subaccount_name}': {e}",
                                              exc_info=True)
                                logging.error(
...
def generate_claude_streaming_response(url, headers, payload, model, subaccount_name):
    ...
    content_block_start_data = {"type": "content_block_start", "index": 0,
                                "content_block": {"type": "text", "text": ""}}
    content_block_start_event = f"event: content_block_start\ndata: {json.dumps(content_block_start_data)}\n\n"
