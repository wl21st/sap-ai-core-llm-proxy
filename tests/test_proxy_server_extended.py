...
from proxy_server import (
    app, format_embedding_response, handle_embedding_service_call,
    load_balance_url, handle_claude_request, handle_gemini_request,
    handle_default_request, get_claude_stop_reason_from_gemini_chunk,
    get_claude_stop_reason_from_openai_chunk, proxy_config
)
...
def setup_test_config():
    ...
    original_tokens = proxy_config.secret_authentication_tokens.copy() if proxy_config.secret_authentication_tokens else []
...
class TestEmbeddingEndpoint:
    ...
    def test_embedding_endpoint_success(self, mock_post, mock_token_manager, 
                                       mock_handle_call, client, setup_test_config):
    ...
    @patch('proxy_server.TokenManager')
    @patch('proxy_server.requests.post')
    def test_embedding_endpoint_http_429_error(self, mock_post, mock_token_manager,
                                               mock_handle_call, client, setup_test_config):
        ...
        mock_headers.__getitem__ = Mock(side_effect=lambda k: "60" if k == "Retry-After" else None)
...
class TestHandleEmbeddingServiceCall:
    ...
    def test_handle_embedding_service_call_basic(self, mock_load_balance, setup_test_config):
...
class TestHandleClaudeRequestExtended:
    ...
    def test_handle_claude_request_37_streaming(self, setup_test_config):
        ...
        url, modified_payload, subaccount = handle_claude_request(payload, "anthropic--claude-4.5-sonnet")
    ...
    def test_handle_claude_request_older_version_streaming(self, setup_test_config):
        ...
        with patch('proxy_server.Detector.is_claude_37_or_4', return_value=False):
            url, modified_payload, subaccount = handle_claude_request(payload, "claude-3.5-sonnet")
...
class TestHandleGeminiRequestExtended:
    ...
    def test_handle_gemini_request_streaming(self, setup_test_config):
        ...
        url, modified_payload, subaccount = handle_gemini_request(payload, "gemini-2.5-pro")
    ...
    def test_handle_gemini_request_model_with_colon(self, setup_test_config):
        ...
        proxy_config.subaccounts["test-sub"].normalized_models["gemini-pro:latest"] = ["https://test.com"]
        proxy_config.model_to_subaccounts["gemini-pro:latest"] = ["test-sub"]
        
        url, modified_payload, subaccount = handle_gemini_request(payload, "gemini-pro:latest")
...
class TestHandleDefaultRequestExtended:
    ...
    def test_handle_default_request_o3_model(self, setup_test_config):
        ...
        payload = {"messages": [{"role": "user", "content": "test"}], "temperature": 0.7}
        
        # Add o3 model
        proxy_config.subaccounts["test-sub"].normalized_models["o3-mini"] = ["https://test.com"]
...
class TestEventLoggingEndpoint:
    ...
    def test_event_logging_post(self, client):
        ...
        response = client.post('/api/event_logging/batch',
                              json={"events": []})
        assert response.status_code == 200
...
class TestProxyClaudeRequestEndpoint:
    ...
    def test_proxy_claude_request_no_auth(self, mock_validator, client, setup_test_config):
    ...
    def test_proxy_claude_request_non_claude_fallback(self, mock_is_claude, mock_load_balance,
                                                       mock_validator, client, setup_test_config):
        """Test Claude endpoint with non-Claude model falls back."""
        mock_validator_instance = Mock()
        mock_validator_instance.validate.return_value = True
        mock_validator.return_value = mock_validator_instance
        
        mock_load_balance.return_value = ("https://test.com", "test-sub", "test-rg", "gpt-4")
...
class TestHandleNonStreamingRequest:
    ...
    @patch('proxy_server.requests.post')
    def test_handle_non_streaming_request_success_claude(self, mock_post, client, setup_test_config):
    ...
    @patch('proxy_server.requests.post')
    def test_handle_non_streaming_request_json_error(self, mock_post, client, setup_test_config):
    ...
    @patch('proxy_server.requests.post')
    def test_handle_non_streaming_request_http_error(self, mock_post, client, setup_test_config):
...
class TestGenerateStreamingResponse:
    ...
    @patch('proxy_server.requests.post')
    def test_generate_streaming_response_openai(self, mock_post, client, setup_test_config):
    ...
    @patch('proxy_server.requests.post')
    def test_generate_streaming_response_claude(self, mock_post, client, setup_test_config):
...
class TestHandleClaudeRequest:
    ...
    def test_handle_claude_request_with_temperature(self, setup_test_config):
        ...
        assert "inferenceConfig" in modified_payload or "temperature" in modified_payload
...
class TestEmbeddingEndpointEdgeCases:
    ...
    @patch('proxy_server.TokenManager')
    @patch('proxy_server.requests.post')
    def test_embedding_endpoint_array_input(self, mock_post, mock_token_manager,
                                            mock_handle_call, client, setup_test_config):
...
class TestConvertersEdgeCases:
    ...
    def test_convert_claude_to_openai_with_tools(self):
        ...
        result = Converters.convert_claude_to_openai(claude_response, "claude-3.5-sonnet")
