...
class TestGetSAPAICoreSDKSession:
    ...
    def test_session_initialization_logs_message(self):
        ...
        with (
            patch("proxy_server.Session") as mock_session_class,
...
class TestGetSAPAICoreSDKClient:
    ...
    def test_client_creation_on_first_call_for_model(self, mock_config):
        ...
        with patch("proxy_server.get_sapaicore_sdk_session", return_value=mock_session):
            ...
            mock_session.client.assert_called_once_with(
                model_name="claude-3-opus", config=expected_config
            )
    ...
    def test_different_clients_for_different_models(self):
        ...
        with patch("proxy_server.get_sapaicore_sdk_session", return_value=mock_session):
            ...
            assert (
                proxy_server._bedrock_clients["claude-3-sonnet"] == mock_client_sonnet
            )
    ...
    @patch("proxy_server.Config")
    def test_client_cache_returns_none_check(self, mock_config):
...
class TestSDKSessionAndClientIntegration:
    ...
    @patch("proxy_server.Config")
    def test_client_uses_session_correctly(self, mock_config):
